...
def train_step(state, batch):
    def loss_fn(params):
        ...
        preds_lr = rnet_lr.apply({'params': rnet_lr_params}, inputs_agent, args.model.split('_')[1], 'lr')
        preds_map = rnet_hr.apply({'params': rnet_hr_params}, inputs_map, args.model.split('_')[1], 'hr')
        preds_sample = rnet_hr.apply({'params': rnet_hr_params}, inputs_sample, args.model.split('_')[1], 'hr')
...
def test_step(state, batch):
    ...
    preds_lr = rnet_lr.apply({'params': rnet_lr_params}, inputs_agent, args.model.split('_')[1], 'lr')
    preds_hr = rnet_hr.apply({'params': rnet_hr_params}, inputs, args.model.split('_')[1], 'hr')
...
rnet_hr_params = rnet_hr.init(rng, jnp.ones([1, 3, 224, 224]))['params']
rnet_lr_params = rnet_lr.init(rng, jnp.ones([1, 3, 224, 224]))['params']
...
if args.ckpt_hr_cl is not None:
    rnet_hr_params = restore_checkpoint(args.ckpt_hr_cl, rnet_hr_params)
    print('loaded the high resolution classifier')

if args.ckpt_lr_cl is not None:
    rnet_lr_params = restore_checkpoint(args.ckpt_lr_cl, rnet_lr_params)
